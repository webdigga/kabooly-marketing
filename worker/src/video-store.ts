...
// Makes the vertical starting image (logo drawn in, like the platform
// images), starts the video from it in Google's background mode, and
// records the job as pending, replacing any earlier video for the advert.
// Throws if either step fails; the caller refunds the lease.
export async function requestVideo(env: Env, req: VideoRequest): Promise<VideoRow> {
  const { advert, profile } = req;
  const logo = await loadLogo(env, profile);
  const raw = await generateImage(env, videoStartPrompt(profile, advert.topic, logo !== null), VIDEO_SHAPE.aspectRatio, logo);
  const start = await fitToShape(env, raw, VIDEO_SHAPE);
  const interactionId = await startVideo(env, start, logo, videoPrompt(profile, advert.topic, req.motion, logo !== null));
