...
Done:   2026-09-17, deployed: account emails are HTML with the shared
        Kabooly signature (src/email-layout.ts). Self-service account
        deletion removed; Settings says "To cancel or delete your account,
...
- D1 (`DB`) via drizzle; R2 (`FILES`) for logos, uploads, advert images and videos (served with byte ranges); Cloudflare Images (`IMAGES`) crops Gemini output and brands own photos; Durable Object (`LIMITER`, one per account) for limits.
