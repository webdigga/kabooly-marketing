...
- Own-photo posts (2026-09-17): upload rules are JPEG, PNG or WebP, at least 1200 pixels on the shortest side, up to 20 MB, checked in the browser and again by the Worker. HEIC is not accepted: iPhones convert to JPEG when a photo is picked in the browser. One pending upload per account; the photo is deleted once its platform images are cut, so own-photo images cannot be regenerated. Cloudflare Images crops with `gravity: auto` and draws the real logo on a white strip with a brand-coloured edge (the edge is a generated one-pixel PNG, stretched).
