...
- Logos (2026-09-18): the scan keeps the best logo on the page whatever its format, and the Worker converts what it cannot read (AVIF, GIF) into a PNG, falling back to the next candidate only when the conversion fails. Every logo, scanned or uploaded, is then tidied in the browser: a flat background is dropped to transparency and the empty margins cut off. A logo that still brings its own background is clipped to its badge on a Story rather than laid on one.
- Instagram Stories (2026-09-18): a fourth platform, 1080x1920, ticked on the Image advert and Photo post pages (off by default: a Story lasts 24 hours). Gemini makes a full screen photograph with the top and bottom quarters kept clear, Haiku writes a headline and a call to action (`adverts.story_words`), and the browser draws those words, the logo and the button over the photo when the Story is shown or downloaded, clear of Instagram's own controls (250px bottom). Branding is a Story-sized lockup at the foot of the image, not the thin strip a square advert carries: the logo at up to 460x210 with the web address under it, drawn on the darkened photo, on a badge whose shape comes from the logo's proportions (a circle for a square logo, a rounded rectangle for a wordmark) and whose colour comes from the logo's brightness (white behind a dark logo, dark behind a light one), both measured from the logo's own pixels. Nothing is stored: the Story is composed whole in the browser.
